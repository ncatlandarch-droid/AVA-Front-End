...
const CRE_CONFIG = {
  ...
  defaults: {
    ...
    organization: 'Think! Design and Planning, LLC',
...
function collectCREProjectData() {
  ...
  const userName = state.user?.displayName || 'Designer';
  ...
  let description = `${config.name} is a ${metrics.totalAreaAcres || '—'}-acre landscape project at ${config.college || 'the project site'}`;
  description += ` designed through the Adaptive Visualization Assistant (AVA) platform using SITES v2 sustainability scoring.`;
  if (total > 0) {
...
function generateObjectiveHTML(config, metrics, sections, scores, total, tier, elements, biome, designDesc) {
  ...
  html += `
    <div style="background: linear-gradient(135deg, #004684 0%, #002d5a 100%); color: white; padding: 24px 28px; border-radius: 12px; margin-bottom: 20px;">
      ...
      <p style="margin: 0; opacity: 0.85; font-size: 14px;">${config.college || 'Think! Design and Planning'} · AVA Design Assessment</p>
    </div>`;
  ...
  html += `
    <div style="border-top: 2px solid #e9ecef; padding-top: 16px; margin-top: 20px; text-align: center; color: #6c757d; font-size: 12px;">
      <p>Generated by <strong>AVA — Adaptive Visualization Assistant</strong><br>
      Think! Design and Planning, LLC<br>
